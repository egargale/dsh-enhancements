...
In DSH, look for `visual-explainer.config.md` in the session workspace root when the user hasn't named a theme or font. (Upstream also supports `.claude/visual-explainer.local.md` personal overrides for Claude Code — not applicable in DSH.)

These values seed `DEFAULT_THEME` and `DEFAULT_FONT` only. The reader can still change them, and an unrecognized or absent value falls back to what the page's aesthetic direction would have chosen anyway. This is an agent-readable generation contract, not a native renderer parameter — in DSH the page is written with the `write` tool like any other output.
