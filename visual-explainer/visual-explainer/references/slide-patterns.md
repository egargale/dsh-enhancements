...
**Step 4 — Plan visual accents.** DSH has no bundled image generation. Plan CSS gradient and SVG decoration accents instead: a gradient background treatment for the **title slide** (sets the visual tone), a **full-bleed slide** (immersive gradient or SVG composition for a key moment), and small inline SVG accents (icons, sparklines, dividers) on content slides. Prepare these before writing HTML so they're embedded as inline SVG / CSS. See the Proactive Imagery section below. Never error on a missing image tool — CSS and SVG alone must carry the deck.
...
Full-viewport hero. Background treatment via gradient, texture, or SVG composition. 80–120px display type.
...
**DSH has no image generation.** The upstream skill optionally generates AI images via surf-cli/Gemini; that tool does not exist in DSH. Do not attempt to generate images, and never error on a missing image tool. Instead, plan CSS gradient and SVG decoration accents in priority order:

1. **Title slide** (always): gradient or SVG composition that sets the deck's visual tone. Match the topic and palette. Example: an abstract geometric SVG pattern with accent lines for a technical preset.
2. **Full-bleed slide** (always if deck has one): immersive gradient background or full-viewport SVG for the deck's visual anchor moment. Style should match the preset — geometric/abstract for clean presets, circuit-board or terminal aesthetic for Terminal Mono, warm textured treatments for editorial presets.
3. **Content slides with conceptual topics** (1–2 if the deck has room): small inline SVG illustration in the `.slide__aside` area for abstract concepts.

**Build these before writing HTML** so they're ready to embed. Inline SVG (directly in the markup) and CSS `linear-gradient`/`radial-gradient` backgrounds keep the deck self-contained — no external assets, no base64 blobs. Pull colors from the preset's CSS variables so accents match the palette. Use the `.slide__bg--gradient` pattern for bold gradient backgrounds.
...
**When to skip decorative accents:** Pure structural or data-heavy decks (code reviews, table comparisons) may not need them. The deck must stand on CSS, typography, and diagrams alone.
